import React, { useEffect, useState } from "react"; import { toast } from "react-hot-toast"; import { useNavigate } from "react-router-dom"; import { getDatabase, ref, onValue, remove } from "firebase/database"; import { app } from "../firebase";
...
useEffect(() => { const unsubList = []; for (let i = 1; i <= 30; i++) { const tableKey = table_${i}; const tableRef = ref(db, orders/${tableKey}); const unsub = onValue(tableRef, (snapshot) => { setOrdersByTable((prev) => { const updated = { ...prev }; if (snapshot.exists()) { updated[i] = Object.values(snapshot.val()); } else { delete updated[i]; } return updated; }); }); unsubList.push(unsub); }
    ...
    Object.entries(ordersByTable).map(([tableNum, orderList], index) => (
      <div
        ...
        style={{
          ...
          padding: 10,
          borderRadius: 5,
          backgroundColor: isReordered(orderList)
            ? "#ffe4e6"
            : "#f0fdf4",
        }}
      >
