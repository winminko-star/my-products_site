    ...
    <div className="user-panel-container animated-background">
      {showThankYou && (
        <div className="thank-you-popup">
          <img src="/images/thankyou.jpg" alt="Thank You" className="thank-you-img" />
        </div>
      )}
            ...
            <div className="item-grid">
              {products.filter((item) => item.category === cat).map((item) => (
                <button key={item.id} className="product-btn" onClick={() => addToCart(item)} disabled={checkedOut}>
                  <img src={item.image || "/images/default.png"} alt={item.name} className="product-img" />
                  <span className="product-name">{item.name}</span>
                  <span className="product-price">{item.price.toLocaleString()} Ks</span>
                </button>
              ))}
            </div>
          </div>
              ...
              {cart.map((item) => (
                <tr key={item.id}>
                  <td className="cart-item-name">
                    <img src={item.image || "/images/default.png"} alt={item.name} className="cart-item-img" />
                    {item.name}
                  </td>
                  <td>{item.unit}</td>
                  <td>
                  ...
                  <td>
                    <button onClick={() => removeFromCart(item.id)} className="delete-btn" title="Remove" disabled={checkedOut}>❌</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <div className="total-amount">Total: {totalAmount.toLocaleString()} Ks</div>
        </div>
