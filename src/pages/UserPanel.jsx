          ...
          {products.filter((item) => item.category === cat).map((item) => (
            <button key={item.id} className="product-btn" onClick={() => addToCart(item)} disabled={checkedOut}>
              <img
                ...
                alt={item.name}
                style={{ width: 50, height: 50, borderRadius: "50%", objectFit: "cover", marginBottom: 6 }}
              />
              <span style={{ fontWeight: "bold", textAlign: "center" }}>{item.name}</span>
              <span style={{ fontSize: 14, color: "#eee", textAlign: "center" }}>
            ...
            <tr key={item.id}>
              ...
              <td>
                <button
                  ...
                  style={{ background: "transparent", border: "none", color: "red", fontSize: 16, cursor: "pointer" }}
